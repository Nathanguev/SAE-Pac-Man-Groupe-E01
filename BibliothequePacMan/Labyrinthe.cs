...
namespace Bibliotheque_PacMan
{
    public class Labyrinthe // Classe Labyrinthe
    {
        ...
        public void affichagePosistionsCellules() // Fonction de débogage qui affiche la position x, y de chaque cellule dans le labyrinthe
        {
            int index = 0;

            for (int i = 0; i < hauteur; i++)
            {
                affichageLigneTab(index);
                Console.WriteLine("+");
                for (int j = 0; j < largeur; j++)
                {
                    Console.Write($"|{cellules[index].affiche()}");
                    index++;
                }
                Console.WriteLine("|");
            }
        ...
        private void affichageForm()
        {
            ...
            for (int i = 0; i < hauteur; i++)
            {
                for (int j = 0; j < largeur; j++)
                {
                    if (cellules[index].getVoisins().Count == 4)
                    {

                    }
                    else if (cellules[index].getVoisins().Count == 3)
                    {
        ...
        private void affichageLigneTab(int index) // Fonction d'affichage qui s'occupe d'afficher les lignes horizontales du labyrinthe
        {
            for (int i = 0; i < largeur; i++)
            {
                if (index >= largeur)
                {
                    if (cellules[index].isLien(cellules[index - largeur]))
                    {
                        Console.Write("+   ");
                    }
                    else
        ...
        private void affichageColonneTab(int index) // Fonction d'affichage qui s'occupe d'afficher les lignes verticales (colonnes) du labyrinthe
        {
            if (index >= 1)
            {
                if (cellules[index].isLien(cellules[index - 1]))
                {
                    Console.Write($"    ");
                }
                else
        ...
        public void affichageVoisins() // Fonction de débogage pour afficher la liste des voisins
        {
            for (int i = 0; i < hauteur * largeur; i++)
            {
                Console.Write($"Voisins de {cellules[i].affiche()} : ({cellules[i].getVoisins().Count})");
                for (int j = 0; j < cellules[i].getVoisins().Count; j++)
                {
                    Console.Write($" ; {cellules[i].getVoisins()[j].affiche()}");
                }
                Console.WriteLine();
            }
        }

        public void affichageLiens() // Fonction de débogage pour afficher la liste des liens
        {
            for (int i = 0; i < hauteur * largeur; i++)
            {
                Console.Write($"Liens de {cellules[i].affiche()} : ({cellules[i].getLiens().Count})");
                for (int j = 0; j < cellules[i].getLiens().Count; j++)
                {
                    Console.Write($" ; {cellules[i].getLiens()[j].affiche()}");
                }
                Console.WriteLine();
            }
        }
        ...
        public void affiche() // Fonction d'affichage qui s'occupe d'afficher le labyrinthe entier dans la console
        {
            int index = 0;
